...
from .session import render_case_log

def generate_html(state: ScoringSessionState, output_path: str | None = None) -> str:
    out = Path(output_path) if output_path else Path("sessions") / "scoring" / state.session_id / "report.html"
    cases = []
    for c in state.cases:
        cases.append(f"<article><h2>Case {html.escape(c.id)} — {html.escape(c.finding_type.value)}</h2><p><b>Status:</b> {html.escape(c.status)}</p><h3>Illustrative response</h3><pre>{html.escape(c.response)}</pre><h3>Guide passages</h3><p>{html.escape(' | '.join(c.guide_passages))}</p><h3>Interpretation</h3><p>{html.escape(c.reasoning)}</p><p><b>Guide code:</b> {html.escape(c.likely_code or 'Unresolved')} &nbsp; <b>Proposed code:</b> {html.escape(c.proposed_code or 'Unresolved')}</p><p><b>Uncertainty:</b> {html.escape(c.uncertainty or 'None recorded.')}</p></article>")
